...
func (h *MarketingHandler) CreateCreative(w http.ResponseWriter, r *http.Request) {
	...
	if c.S3FilePath != nil && *c.S3FilePath != "" {
		key := utils.ParseS3KeyFromPath(*c.S3FilePath)
		exists, err := h.s3.ObjectExists(r.Context(), key)
		if err != nil {
			utils.WriteError(w, 500, "Failed to validate S3 object")
			return
		}
		if !exists {
			utils.WriteError(w, 400, "S3 object not found; upload file first")
			return
		}
		*c.S3FilePath = key
	}
...
func (h *MarketingHandler) PatchCreative(w http.ResponseWriter, r *http.Request) {
	...
	if rawPath, ok := patch["s3_file_path"]; ok {
		if p, ok := rawPath.(string); ok && p != "" {
			key := utils.ParseS3KeyFromPath(p)
			exists, err := h.s3.ObjectExists(r.Context(), key)
			if err != nil {
				utils.WriteError(w, 500, "Failed to validate S3 object")
				return
			...
			if !exists {
				utils.WriteError(w, 400, "S3 object not found")
				return
			}
			patch["s3_file_path"] = key
		}
	}
...
func (h *MarketingHandler) GetUploadURL(w http.ResponseWriter, r *http.Request) {
	...
	key := h.s3.BuildObjectKey(body.FileName)
	uploadURL, expiresIn, err := h.s3.PresignPutObject(r.Context(), key, body.ContentType)
	if err != nil {
		utils.WriteError(w, 500, "Could not generate upload URL")
		return
	}
	utils.WriteJSON(w, 200, map[string]interface{}{"upload_url": uploadURL, "s3_file_path": key, "expires_in": expiresIn})
}
