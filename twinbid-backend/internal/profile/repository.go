...
func (r *Repository) Update(ctx context.Context, userID string, u models.User) (models.User, error) {
	row := r.db.QueryRowContext(ctx, `
		UPDATE users SET login=$2, mail=$3, name=$4, telegram=$5, manager_telegram=$6, balance=$7, timezone=$8,
			...
			campaign_balance_notifications=$12, balance_treshold=$13, updated_at=NOW()
		WHERE id=$1
	...
	`, userID, u.Login, u.Mail, u.Name, u.Telegram, u.ManagerTelegram, u.Balance, u.Timezone,
		u.EmailNotifications, u.CampaignStatusNotifications, u.LowBalanceNotifications, u.CampaignBalanseNotifications, u.BalanceTreshold)
...
const selectUser = `SELECT id, login, mail, name, telegram, manager_telegram, balance, timezone,
	email_notifications, campaign_status_notifications, low_balance_notifications, campaign_balance_notifications, balance_treshold FROM users`
