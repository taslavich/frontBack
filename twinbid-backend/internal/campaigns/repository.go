...
func (r *Repository) Create(ctx context.Context, c models.Campaign) (models.Campaign, error) {
	row := r.db.QueryRowContext(ctx, `
		INSERT INTO campaigns (
			...
			start_ts, end_ts, active_intervals, country, language, device_type, os, browser, site_id, ip
		) VALUES (
	...
	`, c.UserID, c.CampaignName, c.FormatType, c.BrandName, c.H, c.W, c.Status, c.TrafficType, pq.Array(c.Vertical), c.PricingModel,
		...
		jsonArg(c.ActiveIntervals), jsonArg(c.Country), jsonArg(c.Language), jsonArg(c.DeviceType), jsonArg(c.OS), jsonArg(c.Browser), jsonArg(c.SiteID), jsonArg(c.IP))
...
func (r *Repository) Update(ctx context.Context, c models.Campaign) (models.Campaign, error) {
	row := r.db.QueryRowContext(ctx, `
		UPDATE campaigns SET
			...
			country=$20, language=$21, device_type=$22, os=$23, browser=$24, site_id=$25, ip=$26, updated_at=NOW()
		WHERE user_id=$1 AND campaign_id=$2
		RETURNING campaign_id, user_id, campaign_name, format_type, brand_name, h, w, status, traffic_type, vertical,
			pricing_model, base_price_cpm, base_price_cpc, evenness_by_slot_mode, goal_total_dollars, cum_done_dollars,
			start_ts, end_ts, active_intervals, country, language, device_type, os, browser, site_id, ip
	`, c.UserID, c.CampaingID, c.CampaignName, c.FormatType, c.BrandName, c.H, c.W, c.Status, c.TrafficType, pq.Array(c.Vertical),
		c.PricingModel, c.BasePriceCPM, c.BasePriceCPC, c.EvennessBySlotMode, c.GoalTotalDollars, c.CumDoneDollars,
		c.StartTS, c.EndTS, jsonArg(c.ActiveIntervals), jsonArg(c.Country), jsonArg(c.Language), jsonArg(c.DeviceType), jsonArg(c.OS), jsonArg(c.Browser), jsonArg(c.SiteID), jsonArg(c.IP))
...
const baseCampaignSelect = `SELECT campaign_id, user_id, campaign_name, format_type, brand_name, h, w, status, traffic_type, vertical,
	pricing_model, base_price_cpm, base_price_cpc, evenness_by_slot_mode, goal_total_dollars, cum_done_dollars,
	start_ts, end_ts, active_intervals, country, language, device_type, os, browser, site_id, ip FROM campaigns`
...
func scanCampaign(s scanner) (models.Campaign, error) {
	...
	err := s.Scan(&c.CampaingID, &c.UserID, &c.CampaignName, &c.FormatType, &brand, &h, &w, &c.Status, &c.TrafficType, &vertical,
		&c.PricingModel, &c.BasePriceCPM, &c.BasePriceCPC, &c.EvennessBySlotMode, &c.GoalTotalDollars, &c.CumDoneDollars,
